# this file was backupe when the Christmas of 2024 ended(2024/12/26 20:06)
...
def normalize(nx_graph, netlist_name, all_tj):
    ...
    for idx, node in enumerate(nx_graph.nodes(data=True)):
        ...
        #  注意：这里对比原留一法代码有针对T攻击数据集的修改
        if re.search('trojan', node[0]) is not None:
            net_tj.append(node[0])
            node[1]['y'] = 1
            if re.search('T1', node[0]):
                attack_info['T1_attack'].append(node[0])
            elif re.search('T2', node[0]):
                attack_info['T2_attack'].append(node[0])
            elif re.search('T3', node[0]):
                attack_info['T3_attack'].append(node[0])

        elif netlist_name in special_files:
        ...
        type_of_node = nodetype
        # net_attack_info = {netlist_name: attack_info}
...
def get_all_kinds_of_gate(inpath, file, outpath):
    all_gate_kinds = set()
    print("getting_all_kinds_of_gate")
...
if __name__ == "__main__":
    ...
    attack_files_inpath = os.getcwd() + '/AEattack_datasets/'
    inpath = os.getcwd() + relapath
    outpath = os.getcwd() + '/FeaVec/20dim/'
    #  获得所有门级网表中门电路节点种类

    global_node_types = get_all_kinds_of_gate(inpath, files, outpath)
    global_node_types2 = get_all_kinds_of_gate(attack_files_inpath, attack_files, outpath)
    global_node_types = global_node_types2.union(global_node_types)
    ...
    # 下面与上面的不同：all_tj_attack
    data_list = []
    attack_info = {}
    for i in range(len(attack_files)):
        # if 1:
        gate, edge, gate_info, Input, Output, width, dicofAssign = infoExtracion(
            attack_files_inpath + attack_files[i], outpath, 1)
        #   infoExtracion()返回信息：  return gate, edge, gate_info, Input, Output, width, dicofAssign
        ...
        attack_info.update({attack_files[i]:normalize(nxgraph, attack_files[i], all_tj_attack)})
        data_pre = from_networkx(nxgraph)
        x = F.one_hot(data_pre.x, num_classes=num_feature_dim).float()

        data = Data(x=x, edge_index=data_pre.edge_index, y=data_pre.y)
        data.netname = attack_files[i]
        data_list.append(data)
    ...
    # print(attack_info)
    print("攻击后网表中，根据节点名识别到的各网表木马节点：")
    for metrics_dict in all_tj_attack:
        for sample_name, metrics in metrics_dict.items():
            print(f'{sample_name}:{len(metrics)} {metrics}')
            print(attack_info[sample_name])

    attack_dataset = data_list
    ...
    for train_index, test_index in loo.split(dataset):
        ...
        train_data = [dataset[i] for i in train_index]  # 修改的
        #  下面：使用攻击后的文件作为评估集
        val_data = [attack_dataset[i] for i in test_index]  # 修改的
